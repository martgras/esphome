...
// don't (and can't) syncronize their requests.
// if a component sends a request while another component is waiting for a response
// after reading from uart it's not possible to identitfy to which request a response byte belongs to.
// introducing a queue allows enforcing the serialization at the modbus level without breaking existing
// components building on modbus or forcing them to check the state of modbus (idle / waiting for response)
// the queue is checked in loop() and queued requests are send when no other request is pending
