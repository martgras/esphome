...
scd_co2_ns = cg.esphome_ns.namespace("scd_co2")
SCDCO2Component = scd_co2_ns.class_(
    "SCDCO2Component", cg.PollingComponent, i2c.I2CDevice
)
SensorType = scd_co2_ns.namespace("SensorType")
SENSOR_TYPE = {
    "scd30": SensorType.SCD30,
    "scd40": SensorType.SCD40,
    "scd41": SensorType.SCD41,
}
...
CONFIG_SCHEMA = (
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(SCDCO2Component),
...
async def to_code(config):
    ...
    if config[CONF_TYPE] == "scd40" or config[CONF_TYPE] == "scd41":
        cg.add_define("SCD4x")
