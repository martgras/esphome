...
bool SensirionI2CDevice::get_register(uint16_t reg, uint16_t *data, uint8_t len, uint8_t delay_ms) {
  if (!this->write_command(reg)) {
    ESP_LOGE(TAG, "Failed to write i2c register=0x%X err=%d,", reg, this->last_error_);
    return false;
  }
  delay(delay_ms);
 ...
 */
bool SensirionI2CDevice::write_command_(uint16_t command, uint8_t command_len, const uint16_t *data, uint8_t data_len) {
