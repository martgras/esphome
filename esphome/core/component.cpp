...
void Component::set_retry(const std::string &name, uint32_t initial_wait_time, uint8_t max_retries,  // NOÖINT
                          std::function<RetryResult()> &&f, bool delay_first_execution,              // NOLINT
                          float backoff_increase_factor) {                                           // NOLINT
  App.scheduler.set_retry(this, name, initial_wait_time, max_retries, std::move(f), delay_first_execution,
...
void Component::set_retry(uint32_t initial_wait_time, uint8_t max_retries, std::function<RetryResult()> &&f,  // NOLINT
                          bool delay_first_execution, float backoff_increase_factor) {                        // NOLINT
  App.scheduler.set_retry(this, "", initial_wait_time, max_retries, std::move(f), delay_first_execution,
                          backoff_increase_factor);
}
